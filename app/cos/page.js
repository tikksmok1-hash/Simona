  ...
  if (cart.length === 0) {
    return (
      <div className="min-h-screen bg-white pt-[112px] md:pt-[170px] flex flex-col items-center justify-center text-center px-4">
        ...
        <h1 className="font-serif text-3xl font-light text-black mb-3">{t('cartPage.emptyTitle')}</h1>
        <p className="text-neutral-500 text-sm mb-8 max-w-sm">{t('cartPage.emptyDesc')}</p>
        <Link
          href="/"
    ...
    <div className="min-h-screen bg-white pt-[112px] md:pt-[170px]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-10 border-b border-neutral-100 pb-6">
          <h1 className="font-serif text-4xl font-light text-black">{t('cart.myCart')}</h1>
          <p className="text-neutral-500 text-sm mt-1">{cartCount} {cartCount === 1 ? t('cart.product') : t('cart.products')}</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            {cart.map((item) => (
              <div key={item.key} className="flex gap-5 border-b border-neutral-100 pb-6">
                ...
                <div className="relative w-24 h-32 flex-shrink-0 bg-neutral-100 overflow-hidden">
                  {item.image ? (
                    <Image src={item.image} alt={localize(item, 'name', lang)} fill className="object-cover" sizes="96px" />
                  ) : (
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      ...
                      />
                      <span className="text-xs text-neutral-500">{localize(item, 'colorName', lang)}</span>
                      <span className="text-xs text-neutral-300">|</span>
                      <span className="text-xs text-neutral-500">{t('cart.size')}: {item.size}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-3">
                    ...
                    <div className="text-right">
                      ...
                      {item.quantity > 1 && (
                        <p className="text-xs text-neutral-400">{item.price} MDL {t('cart.perPiece')}</p>
                      )}
                    </div>
                  </div>
                </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-neutral-50 border border-neutral-100 p-6 sticky top-[120px]">
              ...
              {/* Delivery method */}
              <div className="mb-5">
                <p className="text-xs text-neutral-500 tracking-widest uppercase mb-3">{t('cartPage.deliveryMethod')}</p>
                <div className="space-y-2">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-xs tracking-widest uppercase font-medium">{t('cartPage.standardDelivery')}</p>
                        <p className={`text-[10px] mt-0.5 ${deliveryMethod === 'standard' ? 'text-white/60' : 'text-neutral-400'}`}>{t('cartPage.standardDays')}</p>
                      </div>
                    </div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-xs tracking-widest uppercase font-medium">{t('cart.pickup')}</p>
                        <p className={`text-[10px] mt-0.5 ${deliveryMethod === 'pickup' ? 'text-white/60' : 'text-neutral-400'}`}>{t('cartPage.pickupAddress')}</p>
                      </div>
                    </div>
              ...
              <div className="space-y-3 text-sm mb-6">
                ...
                <div className="flex justify-between text-neutral-600">
                  <span>
                    {deliveryMethod === 'pickup' ? t('cart.pickup') : `${t('cart.delivery')} ${t('cart.standard')}`}
                  </span>
                  <span className={deliveryMethod === 'pickup' ? 'text-green-600 font-medium' : ''}>
                    {deliveryMethod === 'pickup' ? t('cart.free') : `${deliveryCost} MDL`}
                  </span>
                </div>
              ...
              <div className="mt-6 pt-6 border-t border-neutral-200 space-y-2 text-xs text-neutral-500">
                <div className="flex items-center gap-2">
                  ...
                  <span>{t('cartPage.securePay')}</span>
                </div>
                <div className="flex items-center gap-2">
                  <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                  ...
                  <span>{t('cartPage.freeReturn')}</span>
                </div>
              </div>
            </div>
          </div>
