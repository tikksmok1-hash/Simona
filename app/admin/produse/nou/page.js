...
function ProductForm() {
  ...
  // Variant handlers — stable references so VariantCard doesn't re-render on form changes
  ...
  const addVariantImage = useCallback((vIdx) => {
    setVariants((prev) =>
      prev.map((v, i) =>
  ...
  const removeVariantImage = useCallback((vIdx, imgIdx) => {
    setVariants((prev) =>
      prev.map((v, i) =>
  ...
  const addSize = useCallback((vIdx) => {
    setVariants((prev) =>
      prev.map((v, i) => (i === vIdx ? { ...v, sizes: [...v.sizes, { size: 'M', stock: 0 }] } : v))
    );
  }, []);

  const removeSize = useCallback((vIdx, sIdx) => {
    setVariants((prev) =>
